...
def analyze_hotspot_region():
    """Analyze the hotspot region with the most fires and create visualizations"""
    ...
    # Load NetCDF data for FFMC analysis
    print("Loading NetCDF data...")
    nc_file = './ignite_v01_ffmc_supgrid.nc'
    ds = xr.open_dataset(nc_file)

    # Find fire hotspot using clustering
    print("Identifying fire hotspot...")
    coords = fire_data[['fire_x', 'fire_y']].values

    # Use DBSCAN clustering to find dense fire regions
    clustering = DBSCAN(eps=0.1, min_samples=5).fit(coords)
    fire_data['cluster'] = clustering.labels_

    # Find the largest cluster (hotspot)
    cluster_counts = pd.Series(clustering.labels_).value_counts()
    hotspot_cluster = cluster_counts.index[0]  # Largest cluster
    hotspot_fires = fire_data[fire_data['cluster'] == hotspot_cluster].copy()

    print(f"Hotspot region identified with {len(hotspot_fires)} fires")
    ...
    create_hotspot_visualizations(hotspot_fires, ds)
...
def create_hotspot_visualizations(hotspot_fires, ds):
    """Create all hotspot visualizations"""
    ...
    # Convert coordinates to lat/lon for proper mapping context
    # Assuming Austrian coordinate system - convert to approximate lat/lon
    hotspot_fires['lon'] = hotspot_fires['fire_x'] * 0.01 + 11.0  # Approximate conversion
    hotspot_fires['lat'] = hotspot_fires['fire_y'] * 0.01 + 47.0  # Approximate conversion
    ...
    axes[1].plot([min_val, max_val], [min_val, max_val], 'r--', alpha=0.8, linewidth=2, label='1:1 Linie')
    ...
    plt.suptitle('Waldbrand-Hotspot Analyse: Region Tirol\n275 Brände, 2003-2020',
